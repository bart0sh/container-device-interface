...
func TestAnnotationKey(t *testing.T) {
	...
	for _, tc := range []*testCase{
		...
		},
		{
			...
			plugin:  "1234567890123456789012345678901234567890123456789012345678901234",
			invalid: true,
		},
		{
			name:   "valid, simple",
			plugin: "vendor.class",
			key:    AnnotationPrefix + "vendor.class",
		},
		{
			name:   "valid, special characters",
			plugin: "v-e.n_d.or.cl-as_s",
			key:    AnnotationPrefix + "v-e.n_d.or.cl-as_s",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			key, err := AnnotationKey(tc.plugin)
			if !tc.invalid {
...
func TestAnnotateInjection(t *testing.T) {
	type inject = struct {
		plugin  string
		devices []string
	}
	...
	for _, tc := range []*testCase{
		{
			...
			annotations: map[string]string{
				AnnotationPrefix + "vendor.class": "vendor.com/class=device",
			},
			parsed: []string{
				"vendor.com/class=device",
			},
		},
		{
			...
			annotations: map[string]string{
				AnnotationPrefix + "vendor.class": "vendor.com/class=device1,vendor.com/class=device2,vendor.com/class=device3",
			},
			parsed: []string{
				"vendor.com/class=device1",
				"vendor.com/class=device2",
		...
		},
		{
			...
			annotations: map[string]string{
				AnnotationPrefix + "vendor1.class":  "vendor1.com/class=device1",
				AnnotationPrefix + "vendor2.class":  "vendor2.com/class=device1,vendor2.com/class=device2",
				AnnotationPrefix + "vendor3.class2": "vendor3.com/class2=device1,vendor3.com/class2=device2,vendor3.com/class2=device3",
			},
			parsed: []string{
				"vendor1.com/class=device1",
				"vendor2.com/class=device1",
		...
		},
		{
			...
			existing: map[string]string{
				AnnotationPrefix + "vendor3.class2": "vendor3.com/class2=device0",
			},
			injections: []*inject{
				{
					plugin: "vendor1.class",
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, i := range tc.injections {
				...
				annotations, err = AnnotateInjection(annotations, i.plugin, i.devices)
				if !tc.invalid {
	...
	for _, tc := range []*testCase{
		{
			...
			annotations: map[string]string{
				AnnotationPrefix + "vendor.class": "vendor.com/class=device1",
			},
			devices: []string{
				"vendor.com/class=device1",
			},
		...
		},
		{
			name: "invalid, malformed device reference",
			annotations: map[string]string{
				AnnotationPrefix + "vendor1.class1": "vendor1.com/class1=device1",
				AnnotationPrefix + "vendor2.class2": "vendor2.com/class2=device2",
				AnnotationPrefix + "vendor3.class":  "vendor3.com=device3",
			},
			invalid: true,
		},
		{
			name: "invalid, pre-resolved device",
			annotations: map[string]string{
				AnnotationPrefix + "vendor1.class2": "vendor1.com/class2=device1,vendor1.com/class2=device2",
				AnnotationPrefix + "vendor2.class":  "/dev/null",
			},
			invalid: true,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			devices, err := ParseAnnotations(tc.annotations)
			if !tc.invalid {
